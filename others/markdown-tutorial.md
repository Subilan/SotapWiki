...
Markdown 中的代码分为两类，一类是诸如`这样`的代码「块」，使用`\``表示（这个符号在键盘左上角<kbd>Esc</kbd>的下面，与 `~` 同键位。
...
如果您想要上传图片，该怎么办？前往这里：[sm.ms](//sm.ms)，上传您的图片后，会得到一张地址，使用上述的方式将它复制过来，就可以使用了。除此之外，您还可以链接到其它地方的图片。
...
一个表格分为「表头」和「表体」两个部分，Markdown 中的表格也只需要指定这两个部分就可以创建。一个完整的表格长这样：
