...
### 当心天上的幽灵、地上的恶人
在主世界，到晚上或雷雨天，幻翼（俗称“修仙蝠”）会在**连续3个游戏日（约一小时）不睡觉、且头顶无遮荫**的玩家头上生成。
...
- 这里死亡扣款！`/back`、`/home`、`/tpa`、`/sethome` 命令扣款！
- 主服 **mobGriefing** 为开启状态，也就是说苦力怕可以破坏地形，末影人可以搬方块。重要建筑请圈地保护。
- 主服开启玩家 pvp，但禁止恶意 pvp 与骚扰。
- 服务器不提供地图种子，并安装了检测类似「地图种子计算 mod」的插件，发现玩家使用立刻将玩家t出服务器。
- 主服由于考虑到服务器的负载问题，在一定区域以内（6方块为半径）中超过一定数量的动物以后（一般是 5 ~ 6 只）就会在该区域**禁止动物生成**。无论是手动繁殖动物还是用刷怪蛋都会导致没有新幼崽或新成年动物生成。遇到这种情况请扩建您的饲养厂面积，降低区域内动物的密度来重新恢复该区域的动物生成。
