...
?> ❓ **步进价示例**<br>
步进价是指在拍卖的过程中，拍卖者提升价格的最小值。例如 `/hauc 10 6`，那么此物品的起步价为 10，步进价为 6。参与者必须出价 16 元以上。如果有人使用`hbid` 最低出价，那么就是出价 16 元。这个时候抬价必须为 22 元以上，例如`hbid 100`，直接出价 100 而无视步进价。

?> 👆 **保留价示例**<br>
保留价并非必填项。保留价是指拍卖成功的最低价，假如拍卖最后低于这个价钱则视为流拍。
